...
message.channel.sendMessage('**تـم إنـشاء روم كـتابـي**:white_check_mark:')
...
if (message.content.startsWith("^cv")) {
    ...
    message.channel.sendMessage('**تـم إنـشاء روم صـوتي**:white_check_mark: ')
...
if (command == "embed") {
  ...
  client.on("message", (message) => {
    if (message.content.startsWith('^delete')) {
        ...
        if (!channel) return message.reply('**لايوجد روم بهذا الأسم :heavy_multiplication_x:**').catch(console.error);
        channel.delete()
    }
});
...
client.on('message', message => {
  ...
  client.on('message', message => {
    ...
    client.on('message', message => {
      ...
      .addField(':closed_book:| عدم الازعاج',`${message.guild.members.filter(m=>m.presence.status == 'dnd').size}`)
      .addField(':orange_book:| غير موجود',`${message.guild.members.filter(m=>m.presence.status == 'idle').size}`)
...
client.on('ready', () => {
   client.user.setGame(" ^help | By xRoKz ");
}); 
...
client.login('NDk5MjM5NjE0OTIyNDg5ODU2.Dp5ZxA.HDyFnYdic6cxXj98qAb5T-GFmTQ');
